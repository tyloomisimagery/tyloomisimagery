...
function updatePageWithData(sheetData) {
    // Header 
    $('#header-about').text(sheetData.A3);
    $('#header-portfolio').text(sheetData.B3);
    $('#header-pricing').text(sheetData.C3);
    $('#header-contact').text(sheetData.D3);

    // Carousel
    addCarouselImages(sheetData);

    // Links 
    $('#link-1').text(sheetData.A11);
    $('#link-2').text(sheetData.B11);

    // Social
    $('#social-email').attr('href', `mailto:${sheetData.A15}`);
    $('#social-instagram').attr('href', sheetData.A19);
    $('#social-facebook').attr('href', sheetData.B19);

    // Footer
    $('#email').text(sheetData.A15);
    $('#copyright-text').text(sheetData.B15);
...
function addCarouselImages(sheetData) {
    const keys = Object.keys(sheetData).filter(key => key.includes('7') && !key.includes('1') && !key.includes('2'));
